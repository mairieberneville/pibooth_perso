...
class IntroBackground(Background):
    ...
    def resize(self, screen):
        ...
        if self._need_update and self.arrow_location != ARROW_HIDDEN:
            if self.arrow_location == ARROW_TOUCH:
                ...
                self.left_arrow = pictures.get_pygame_image("intro.png", size, vflip=False, color=self._text_color)

                #x = int(self._rect.width * 0.2)
                #y = int(self._rect.height // 2)
                # 🔸 Positionnement : centré horizontalement et plus bas sur l’écran
                x = int((self._rect.width - self.left_arrow.get_rect().width) / 2)
                y = int(self._rect.height * 0.60)  # 60% de la hauteur (descend un peu)
    ...
    def resize_texts(self):
        ...
        # Décale le texte vers le bas
        rect.top += int(self._rect.height * 0.25)
